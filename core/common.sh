...
show_help() {
    ...
    echo -e "  ${GREEN}switch${NC} <app>          Switch active app context"
    echo ""
    ...
    echo ""
    echo -e "${CYAN}Quick Start (Registry):${NC}"
    echo "  1. Search:  tfgrid-compose search"
    echo "  2. Deploy:  tfgrid-compose up wordpress"
    echo "  3. List:    tfgrid-compose list"
    echo "  4. Switch:  tfgrid-compose switch ai-agent"
    echo ""
    echo -e "${CYAN}Examples:${NC}"
    echo "  tfgrid-compose search                    # Browse all apps"
    echo "  tfgrid-compose search wordpress          # Search for WordPress"
    echo "  tfgrid-compose up wordpress              # Deploy from registry"
