...
show_help() {
    ...
    echo -e "  ${GREEN}whitelist${NC} [command]   Manage preferred nodes/farms"
    echo -e "      nodes <ids>           Set preferred node IDs (e.g., '920,891')"
    echo -e "      farms <names>         Set preferred farm names"
    echo -e "      --status              Show current whitelist"
    echo -e "      --clear               Clear all whitelist entries"
    echo -e "  ${GREEN}blacklist${NC} [command]   Manage nodes/farms to avoid"
    echo -e "      nodes <ids>           Set nodes to avoid (e.g., '615,888')"
    echo -e "      farms <names>         Set farms to avoid"
    ...
    echo -e "  ${GREEN}contracts${NC} <subcommand> Contract management via tfcmd (list/show/delete)"
    ...
    echo -e "  ${GREEN}nodes favorites${NC}       Show your favorite nodes (🟢 online, 🔴 offline)"
    ...
    echo "     tfgrid-compose whitelist farms FastFarm,ReliableFarm"
    echo "     tfgrid-compose blacklist nodes 615,888"
    echo "  2. Preferences auto-apply to all deployments"
    echo "  3. Override with CLI flags when needed:"
    echo "     tfgrid-compose up app --blacklist-node 999"
    echo ""
    ...
    echo "  tfgrid-compose blacklist farms BadFarm   # Avoid problematic farm"
    echo "  tfgrid-compose preferences --status      # View all preferences"
    ...
    echo "  tfgrid-compose up tfgrid-ai-agent --refresh  # Deploy with fresh cache"
    echo "  tfgrid-compose up tfgrid-ai-agent --force    # Complete fresh start"
    ...
    echo "  tfgrid-compose nodes                     # Browse available nodes"
    echo "  tfgrid-compose nodes favorites           # View your favorite nodes"
