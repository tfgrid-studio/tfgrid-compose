...
show_help() {
    ...
    echo -e "      ${GREEN}--cpu${NC} <cores>     Override vCPU cores (e.g., 4)"
    echo -e "      ${GREEN}--mem${NC}, --memory <size> Override memory (MB or 8G, 16G, etc.)"
    echo -e "      ${GREEN}--disk${NC} <size>     Override disk size in GB (e.g., 200 or 200G)"
    ...
    echo -e "      ${GREEN}delete${NC} <id...>    Delete contract(s) by ID"
    echo -e "      ${GREEN}delete --container${NC} <id> Delete by container ID (supports short IDs)"
    echo -e "      ${GREEN}delete --all${NC}      Cancel ALL contracts (⚠️ DANGEROUS!)"
    echo -e "      ${GREEN}orphans${NC}           Find orphaned contracts (not in local state)"
    echo -e "      ${GREEN}orphans --delete${NC}  Delete orphaned contracts"
    echo -e "      ${GREEN}clean -i${NC}          Interactive cleanup (select deployments to delete)"
    echo -e "  ${GREEN}state${NC} <subcommand>    State directory management"
    echo -e "      ${GREEN}list${NC}              List all state directories"
    echo -e "      ${GREEN}clean${NC}             Remove orphaned state directories"
    ...
    echo -e "  ${GREEN}network${NC} <subcommand>  Manage network preferences"
    echo -e "      ${GREEN}prefer${NC} <network>   Set preferred runtime network (wireguard|mycelium)"
    echo -e "      ${GREEN}mode${NC} <mode>        Set provisioning mode (wireguard-only|mycelium-only|both)"
    echo -e "      ${GREEN}get${NC}, ${GREEN}show${NC}         Show current settings"
    echo -e "      ${GREEN}list${NC}, ${GREEN}available${NC}   List available networks and modes"
    echo -e "      ${GREEN}test${NC}, ${GREEN}verify${NC}      Test connectivity to both networks"
    echo ""
    ...
    echo "  4. Delete by container: tfgrid-compose contracts delete --container <id>"
    echo "  5. Find orphans: tfgrid-compose contracts orphans"
    echo "  6. Interactive cleanup: tfgrid-compose contracts clean -i"
    echo "  7. State cleanup: tfgrid-compose state clean"
    echo ""
    ...
    echo "  tfgrid-compose ps                        # List deployed apps"
    ...
    echo "  tfgrid-compose contracts delete <id>     # Cancel a contract"
    echo "  tfgrid-compose contracts delete -c <id>  # Delete by container ID (short ID supported)"
    echo "  tfgrid-compose contracts orphans         # Find orphaned contracts"
    echo "  tfgrid-compose contracts orphans --delete # Delete orphaned contracts"
    echo "  tfgrid-compose contracts clean -i        # Interactive cleanup"
    echo "  tfgrid-compose state list                # List state directories"
    echo "  tfgrid-compose state clean               # Remove orphaned state directories"
    echo ""
