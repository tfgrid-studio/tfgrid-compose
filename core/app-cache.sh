...
cache_needs_update() {
    ...
    local has_cache_version=$(echo "$metadata" | jq -r '.cache_version // empty' | grep -v '^empty
...
cache_app() {
    ...
    if git clone --quiet "$repo_url" "$app_dir" 2>/dev/null; then
        ...
        local has_cache_version=$(echo "$old_metadata" | jq -r '.cache_version // empty' | grep -v '^empty
...
export -f cache_needs_update
 | wc -l)
    local has_commit_hash=$(echo "$metadata" | jq -r '.commit_hash // empty' | grep -v '^unknown
